...
1. char 배열을 sort()로 정렬하면 유니코드 값에 따라 정렬된다.

2. BufferedWriter보다 StringBuilder를 사용하는 것이 더 빠르다.

3. i++ 대신 i--을 작성하면 반대순서로 반복한다. 물론 맨 첫번째 요소도 0이 아니라 길이-1이 되어야 할 것이다.

4. BufferedReader, BufferedWrtier 사용 종료 후 close()를 작성하자.

## 2024.2.10

## SQL

1. NVL 함수: null을 대상으로 어떤 값을 출력할지 처리하는 함수
   NVL2 함수: 앞의 NVL함수는 null이 아닌 경우에는 본래의 값을 출력하지만,
   NVL2를 쓰면 null이 아닌 경우에도 값을 지정한 값으로 사용할 수 있다.

2. ORDER BY 문에서 여러 개의 열을 지정하면, 첫 번째 열로 정렬한 후, 첫 번째 열의 값이 같은 행들에 대해 두 번째 열로 정렬한다. 이를 '다중 정렬'이라고 한다.

3. TO_CHAR(대상, 바꿀 형식) AS 별칭 -> 지정된 형식의 문자열로 변환하는 함수이다.
...
1. LIKE는 보통 쿼리문 WHERE절에 주로 사용되며 부분적으로 일치하는 칼럼을 찾을때 사용된다.

2. _ : 글자숫자를 정해줌 (ex. 컬럼명 LIKE '이윤_')
   % : 글자숫자를 정해주지 않음 (ex. 컬럼명 LIKE '%윤설');

3. Oracle에서는 대소문자를 구분한다.
   ex. Table 과 table은 서로 다른 테이블로 인식한다.

4. LIKE를 쓸 때는 작은따옴표로 문자열을 묶어야 한다.

5. TO_CHAR() 사용 후 AS를 붙여 출력할 문자열을 지정해야한다.

6. IN 을 사용할 때는 괄호 안 문자열들에 작은 따옴표를 붙여야된다.

## 2024.2.13

## Spring

1. 일반적으로 ApplicationContext라는 인터페이서를 스프링 컨테이너라고 한다.
   생성 코드 예시 -> ApplicationContext applicationContext =
   new AnnotationConfigApplicationContext(AppConfig.class);

2. 스프링 컨테이너는 파라미터로 넘어온 설정 클래스 정보를 사용하여 빈을 등록한다.

3. 빈 이름은 항상 다른 이름을 부여해야 한다. 즉, 중복된 이름이 있으면 에러 발생한다.

4. 스프링은 빈을 생성하고, 의존 관계를 주입하는 단계가 나누어져 있다.
   하지만 자바 코드로 스프링 빈을 등록하면 생생성자를 호출하면서 의존관계 주입도 한번에 처리된다.

5. getBeanDefinitionNames() : 스프링에 등록된 모든 빈 이름을 조회한다.
   getBeansOfType(): 해당 타입의 모든 빈을 조회한다.
   getBean() : 빈 이름으로 빈 객체(인스턴스)를 조회한다.
   getRole() 함수로 스프링 빈을 구분할 수 있다.

- ROLE_APPLICATION: 일반적으로 사용자가 정의한 빈
- ROLE_INFRASTRUCTURE: 스프링이 내부에서 사용하는 빈

6. 타입으로 빈을 조회할 때, 같은 타입의 스프링 빈이 두 개 이상이면 오류가 발생한다. 이때는 빈 이름을 지정하자.

7. 부모 타입으로 빈을 조회하면, 자식 타입도 함께 조회한다.
   그래서 모든 자바 객체의 최고 부모인 Object 타입으로 조회하면, 모든 스프링 빈을 조회한다.

8. 빈을 조회할 때, 인터페이스가 아닌 특정 하위 클래스(타입)로 조회하는 것은 "추상화에 의존해야지, 구체화에 의존하면 안된다"는 규칙을 깨므로 추천하지 않는 방법이다.

9. 모든 빈을 조회하는 것과 특정 빈을 요청하는 것은 서로 다른 동작이다.
   ex. DiscountPolicy discountPolicy()는 RateDiscountPolicy()를 리턴하더라도
   findAllBeanByParentType()의 결과값(beansOfType.size()).isEqualTo(2)은 2개인 것이 정상이다.

10. Object로 빈을 조회하면 스프링의 기본 빈도 조회된다.

11. BeanFactory란 스프링 컨테이너의 최상위 인터페이스다. 사용할 일은 거의 없다.
    ApplicationContext가 BeanFactory를 상속하며, 빈 관리기능 + 편리한 부가기능을 제공한다.
    ApplicationContext는 메시지 소스를 활용한 국제화 기능, 환경변수, 애플리케이션 이벤트, 리소스 조회 기능을 제공한다.

12. 스프링 컨테이너는 다양한 형식의 설정 정보를 받아들일 수 있다.
    ex. 애노테이션 기반 자바 코드 설정, XML 설정(최근에는 거의 사용X), 사용자 임의 설정

13. BeanDefinition을 직접 생성해서 스프링 컨테이너에 등록할 수 도 있다. 하지만 실무에서 BeanDefinition을
...
14. AnnotationConfigApplicationContext: 이 방법은 자바 기반의 설정을 사용한다.
    @Configuration이 붙은 클래스를 인자로 받아서 그 클래스에 정의된 @Bean들을 스프링 컨테이너에 등록한다.
    팩토리 메소드를 사용하여 빈을 생성하고 초기화하는 작업을 수행합니다. 팩토리 메소드란, 객체 생성을 캡슐화하는 메소드다.
    이 방법은 XML 기반의 설정보다 타입 안전성을 제공하고, IDE의 지원을 받을 수 있어서 더욱 편리하다.

GenericXmlApplicationContext: 이 방법은 XML 기반의 설정을 사용한다. XML 파일을 인자로 받아서 그 파일에 정의된 빈들을 스프링 컨테이너에 등록한다.

## 2024.2.14

## Spring

1. 스프링 없는 순수한 DI 컨테이너는 요청을 할 때마다 객체를 새로 생성한다 -> 메모리 낭비 발생
   해결방안은 해당 객체가 딱 1개만 생성되고, 공유하도록 설계하면 된다. (싱글톤)

2. 싱글톤이란, 클래스의 인스턴스가 1개만 생성되게 보장하는 디자인 패턴이다.
   딱 1개의 객체 인스턴스만 존재해야 되기 때문에 생성자를 private으로 막아야 된다.

3. isSameAs() : == , isEqualTo() : Equals()

4. 싱글톤 패턴은 여러가지 단점이 있는데, 대표적으로 코드 길이가 길다는 것과 테스트하기 어렵다는 점이다.

5. 스프링 컨테이너는 싱글톤 컨테이너다. 객체를 모두 싱글톤으로 관리한다.

6. AnnotationConfigApplicationContext() 안에 클래스를 넣기만 해도 스프링빈으로 등록된다.

7. 스프링빈은 싱글톤 패턴만 지원하는 것은 아니지만, 99%의 스프링빈은 싱글톤을 사용한다.

8. 싱글톤을 만들 때 스프링 빈의 필드에 공유 값을 설정하면 정말 큰 장애가 발생할 수 있다.

9. 싱글톤 객체(스프링빈 포함)는 항상 무상태(필드가 없음)로 설계해야 한다.

10. AppConfig@CGLIB는 AppConfig의 자식 타입이므로, AppConfig 타입으로 조회 할 수 있다.

11. CGLIB는 바이트코드 조작 라이브러이다. 만약 @Bean이 붙은 메소드가 리턴하는 객체가 스프링 컨테이너에 등록되어 있으면 스프링 컨테이너에서 찾아서 반환하고, 만약 컨테이너에 없으면 기존 로직을 호출 한뒤 객체를 생성하고 스프링 컨테이너에 등록한다. 요약하면 @Bean이 붙은 메소드마다 이미 스프링빈이 존재하면 존재하는 빈을 반환하고, 스프링 빈이 없으면 생성해서 스프링 빈으로 등록하고 반환하는 코드가 동적으로 만들어진다. 덕분에 싱글톤이 보장되는 것이다.

12. AppConfig bean = bean.getClass()로 빈의 클래스를 조회하면 AppConfig의 CGLIB어쩌구저쩌구라고 나온다.
    이것은 내가 만든 AppConfig 클래스가 아니라 스프링이 CGLIB라는 바이트코드 조작 라이브러리를 사용해서 AppConfig를 상속받은 임의의 다른 클래스를 만들고, 이를 스프링 빈으로 등록한 것이다.

13. 그래서 실제 스프링 컨테이너의 스프링빈의 객체 이름이 appConfig 이더라도, 참조하는 주소는 CGLIB 객체다.

14. @Bean만 사용해도 스프링 빈으로 등록되지만, 싱글톤을 보장하지 않는다.

15. 스프링 설정 정보는 항상 @Configuration 을 사용하자.

## 알고리즘

1. 자료형 + ""을 하면 String으로 바꿀 수 있다.
   하지만 명시적인 방법이 아니기 때문에 웬만하면 Integer.toString() 또는 String.valueOf()를 사용하도록 하자.

2. charArray로 반복문을 돌려도 좋고, String으로 반복문을 돌린 후 charAt(i)로 작업을 진행해도 좋다.

3. 문자열끼리 같은지 비교할 때는 == 가 아니라 equals()를 사용해야 한다.

4. 만약 반복문을 통해 찾고자 하는 값을 찾으면 break를 써주도록 하자. 속도가 더 빨라진다.

5. String 대신 StringBuilder를 사용하는 것이 100배나 빠르다고 한다.

## 2024.2.15

## 알고리즘

1. %10을 하면 숫자의 가장 오른쪽 값을 얻을 수 있다.
   /=10을 하면 숫자의 가장 오른쪽 값을 제거한다.

2. 삼항 연산자 사용법 -> 조건 ? 참일 때의 값 : 거짓일 때의 값

3. 반복문 사용 시, 줄바꿈 등의 작업이 필요할 때 스코프를 신경쓰자.
   repeat(n)함수를 통해 문자열을 n번 반복하여 출력할 수 있다.

4. System.arraycopy 는 byte[] 형태의 데이터를 자르거나 연접하기 위해 사용하는 메소드다. System.arraycopy 파라미터는 총 5개 요소가 들어간다.
...
1. StringTokenizer는 split()보다 속도가 빠르다. 단, split()은 정규식을 사용할 수 있기 때문에 복잡한 패턴을 자르려면 split()을 사용하자.

2. BufferedWriter()를 사용하면 flush(),close()를, BufferedReader()를 사용하면 close()를 마지막에 작성해야 한다.

3. 3중 반복문도 가능함

### 스프링

1. 웹 서버는 정적인 파일, WAS는 비즈니스 로직을 수행함.
2. http, tcp/ip 등 업무는 제외하고, 비즈니스 로직만 수행하도록 나온 것이 서블릿이다.
...
1. Math.max()는 두 개의 인자를 받을 수 있지만, 그 이상도 가능하다.
   ex. int max = Math.max(score[0], Math.max(score[1], score[2]));

2. (o2 + o1).compareTo(o1 + o2)는 내림차순 정렬을 의미힌다.

3. 문자열 '2'와 '1'이 있다고 가정하자. 둘을 compare()하면 2는 1보다 크다는 결과가 나온다.
   이것은 문자열 간 대소비교는 문자열을 구성하는 각 문자의 유니코드 값(또는 ASCII 값)을 기반으로 수행되기 때문에 가능한 것이다.

4. 다수 요소가 존재하는 배열에서, 오름차순 정렬 후, n/2의 인덱스에 해당하는 요소는 무조건 가장 많이 등장한 요소다.

5. V value = map.getOrDefault(key, defaultValue);: 찾고자 하는 키의 value를 가져오되, 만약 key가 없으면 defaultvalue를 가져온다.

6. 코테 문제를 풀 때, 특정 요소(객체)에 여러 필드를 부여해야 한다면, 클래스를 만드는 것을 고려해보자.(ex. 나이순 정렬)

7. Arrays.sort(대상, new Comparator<자료형> {}) 에서 <>안에 들어가는 것은 Comparator 인터페이스가 어떠한 타입의 객체들을 비교하기 위해 사용됨을 의미한다.

8. 자바의 Arrays.sort()는 안정 정렬을 보장한다. 안정 정렬은 동일한 값을 가진 요소들의 상대적인 순서가 정렬 전과 정렬 후에 동일하게 유지됨을 의미한다.
   만약 나이순으로 정렬하되, 나이가 같은 경우 입력받은 순서대로 정렬하기를 원한다면, 자동적으로 입력받은 순서대로 정렬된다.

## 2024.4.17

## 알고리즘

1. 배열끼리 비교할 때는 항상 ArrayIndexOutOfBoundException을 조심해야 한다.

2. 배열 요소들의 총합은 N _ (N + 1) / 2 로 구할 수 있다. N은 마지막 요소의 값(인덱스 아님)이다.
   ex. {0,1,2,3} = 3 _ 4 / 2 = 12/6 = 6
   단, 이 공식은 맨 첫번째 값이 0 또는 1일 때만 사용할 수 있다.
   만약 {3,4,5,6}의 총합을 구하고 싶다면
   먼저, 1부터 6(마지막 숫자)까지의 합을 구한다: N _ (N + 1) / 2 = 6 _ (6 + 1) / 2 = 21
   그 다음, 1부터 2(시작 숫자의 이전 숫자)까지의 합을 구한다: 2 \* (2 + 1) / 2 = 3
   마지막으로, 1부터 6까지의 합에서 1부터 2까지의 합을 빼준다: 21 - 3 = 18
   따라서, {3, 4, 5, 6}의 합은 18이다.

## 스프링

1. HandlerMethod의 역할은 클라이언트의 요청을 처리하기 위해 @RequestMapping 어노테이션(또는 이와 유사한 어노테이션들)이 붙은 컨트롤러 클래스의 메소드를 참조하는 역할을 한다.
   HandlerMethod는 컨트롤러 메소드의 수많은 정보를 포함한다. 이 정보에는 메소드가 속한 클래스, 메소드 이름, 메소드 파라미터, 메소드가 반환하는 타입 등이 포함된다.

2. 사용자 로그인 여부 인증기능은 컨트롤러 호출 전에만 호출되면 된다. 따라서 preHandle 만 구현하면 된다.

3. 뷰 템플릿으로 HTML을 생성해서 응답하는 것이 아니라, HTTP API처럼 JSON 데이터를 HTTP 메시지 바디에서 직접 읽거나 쓰는 경우 HTTP 메시지 컨버터를 사용하면 편리하다.

4. HTTP 메시지 컨버터의 종류로는
   ByteArrayHttpMessageConverter,
   StringHttpMessageConverter,
   MappingJackson2HttpMessageConverter 가 있다.
   응답의 경우 Http Accept 헤더와 서버의 컨트롤러 반환 타입 정보를 조합해서 HTTP 메시지 컨버터가 선택된다.

5. canRead() , canWrite() : 메시지 컨버터가 해당 클래스, 미디어타입을 지원하는지 체크
   read() , write() : 메시지 컨버터를 통해서 메시지를 읽고 쓰는 기능

6. HTTP 메시지 컨버터는 HTTP 응답/요청 두 경우 모두 사용된다.

7. 스프링 부트는 다양한 메시지 컨버터를 제공하는데, 대상 클래스 타입과 미디어 타입 둘을 체크해서 사용여부를 결정한
   다. 만약 만족하지 않으면 다음 메시지 컨버터로 우선순위가 넘어간다. 이 과정에서 메시지 컨버터가 메시지를 읽을 수 있는지 확인하기 위해 canRead(), canWrite()를 호출하여 검사한다. 만약 검사를 통과하면 read() 또는 write()를 호출한다.

8. RequestMapping핸들러 어댑터가 컨트롤러(핸들러)를 호출할 때 다양한 파라미터, 애노테이션 정보를 기반으로 전달 데이터를 생성한다. 이것은 ArgumentResolver 덕분에 가능한 것이다.

9. return "helloView"; 처럼 컨트롤러에서 String으로 뷰 이름을 반환해도 잘 동작하는 이유는 ReturnValueHandler 덕분이다.
